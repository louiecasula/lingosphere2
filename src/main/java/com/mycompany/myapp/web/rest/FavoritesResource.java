...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.Favorites}.
 */
@RestController
...
public class FavoritesResource {
    ...
    public ResponseEntity<Favorites> partialUpdateFavorites(
        ...
        Optional<Favorites> result = favoritesRepository
            ...
            .map(existingFavorites -> {
                if (favorites.getTimestamp() != null) {
                    existingFavorites.setTimestamp(favorites.getTimestamp());
                }
                if (favorites.getProficiencyLvl() != null) {
                    existingFavorites.setProficiencyLvl(favorites.getProficiencyLvl());
                }
     ...
     */
    @GetMapping("")
    public List<Favorites> getAllFavorites(@RequestParam(name = "filter", required = false) String filter) {
        if ("userprofile-is-null".equals(filter)) {
            log.debug("REST request to get all Favoritess where userProfile is null");
            return StreamSupport
                .stream(favoritesRepository.findAll().spliterator(), false)
                .filter(favorites -> favorites.getUserProfile() == null)
                .toList();
        }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<Favorites> getFavorites(@PathVariable("id") Long id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteFavorites(@PathVariable("id") Long id) {
