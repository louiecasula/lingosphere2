  ...
  return (
    <div>
      <h2 id="user-profile-heading" data-cy="UserProfileHeading">
        <Translate contentKey="lingosphere2App.userProfile.home.title">User Profiles</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="lingosphere2App.userProfile.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/user-profile/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="lingosphere2App.userProfile.home.createLabel">Create new User Profile</Translate>
          </Link>
        </div>
      </h2>
        ...
        {userProfileList && userProfileList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="lingosphere2App.userProfile.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('targetLanguage')}>
                  <Translate contentKey="lingosphere2App.userProfile.targetLanguage">Target Language</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('targetLanguage')} />
                </th>
                <th>
                  <Translate contentKey="lingosphere2App.userProfile.archive">Archive</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="lingosphere2App.userProfile.favorites">Favorites</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {userProfileList.map((userProfile, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{userProfile.nativeLanguage}</td>
                  <td>{userProfile.targetLanguage}</td>
                  <td>{userProfile.archive ? <Link to={`/archive/${userProfile.archive.id}`}>{userProfile.archive.id}</Link> : ''}</td>
                  <td>
                    {userProfile.favorites ? <Link to={`/favorites/${userProfile.favorites.id}`}>{userProfile.favorites.id}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="lingosphere2App.userProfile.home.notFound">No User Profiles found</Translate>
            </div>
          )
        )}
      </div>
